"""Add durable authenticated Alpaca paper account bindings.

Revision ID: 0013_phase4_account_binding
Revises: 0012_phase4_request_budget
...
revision: str = "0013_phase4_account_binding"
down_revision: str | None = "0012_phase4_request_budget"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_BINDING_TABLE_NAME = "phase4_alpaca_paper_account_bindings"
_HEAD_TABLE_NAME = "phase4_alpaca_paper_account_binding_heads"
_INGRESS_PARENT_INDEX = "ux_phase4_broker_ingress_account_receipt_semantic"
_PERMIT_PARENT_INDEX = "ux_phase4_broker_request_account_permit_semantic"


def upgrade() -> None:
    # These exact-row indexes let each binding authenticate both the durable
    # identity and semantic digest of its request permit and raw ingress fact.
    op.create_index(
        _INGRESS_PARENT_INDEX,
        "phase4_broker_ingress_receipts",
        ["account_id", "receipt_id", "semantic_sha256"],
        unique=True,
    )
    op.create_index(
        _PERMIT_PARENT_INDEX,
        "phase4_broker_request_permits",
        ["account_id", "permit_id", "semantic_sha256"],
        unique=True,
    )
    op.create_table(
        _BINDING_TABLE_NAME,
        ...
        # These are nonsecret references only. Credential values are never
        # represented by a column or canonical payload field in this table.
        ...
        sa.CheckConstraint(
            ...
            name=op.f(
                "ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_predecessor_shape"
            ),
        ),
        sa.CheckConstraint(
            "provider_id = 'alpaca-paper' AND environment = 'paper'",
            name=op.f(
                "ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_provider_scope"
            ),
        ),
        sa.CheckConstraint(
            ...
            name=op.f(
                "ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_provider_uuid"
            ),
        ),
        sa.CheckConstraint(
            ...
            name=op.f(
                "ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_secret_reference"
            ),
        ),
        sa.CheckConstraint(
            "length(resolver_id) BETWEEN 1 AND 128 AND length(resolver_version) BETWEEN 1 AND 128",
            name=op.f(
                "ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_resolver_identity"
            ),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_time_order"),
        ),
        sa.CheckConstraint(
            sa.extract("epoch", sa.column("valid_until"))
            - sa.extract("epoch", sa.column("qualified_at"))
            <= 5,
            name=op.f("ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_max_ttl"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_id_shape"),
        ),
        sa.CheckConstraint(
            ...
            name=op.f("ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_hash_lengths"),
        ),
        sa.CheckConstraint(
            "length(canonical_payload) BETWEEN 2 AND 32768",
            name=op.f("ck_phase4_alpaca_paper_account_bindings_phase4_alpaca_binding_payload_size"),
        ),
        ...
        sa.ForeignKeyConstraint(
            ...
            [
                ...
                f"{_BINDING_TABLE_NAME}.expected_provider_account_id",
        ...
        sa.UniqueConstraint(
            "evidence_sha256",
            name=op.f("uq_phase4_alpaca_paper_account_bindings_evidence_sha256"),
        ),
        sa.UniqueConstraint(
            "semantic_sha256",
            name=op.f("uq_phase4_alpaca_paper_account_bindings_semantic_sha256"),
        ),
    )
    op.create_index(
        "ix_phase4_alpaca_account_bindings_account_qualified",
        _BINDING_TABLE_NAME,
        ["account_id", "qualified_at"],
        unique=False,
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "provider_id = 'alpaca-paper' AND environment = 'paper'",
            name=op.f(
                "ck_phase4_alpaca_paper_account_binding_heads_phase4_alpaca_binding_head_provider_scope"
            ),
        ),
        sa.CheckConstraint(
            ...
            "AND length(expected_provider_account_id) = 36",
            name=op.f(
                "ck_phase4_alpaca_paper_account_binding_heads_phase4_alpaca_binding_head_terminal_shape"
            ),
        ),
        sa.CheckConstraint(
            "last_qualified_at < last_valid_until",
            name=op.f(
                "ck_phase4_alpaca_paper_account_binding_heads_phase4_alpaca_binding_head_time_order"
            ),
        ),
        sa.ForeignKeyConstraint(
            ["account_id"],
...
def downgrade() -> None:
    ...
    if connection.dialect.name == "postgresql":
        connection.exec_driver_sql(
            "LOCK TABLE phase4_alpaca_paper_account_binding_heads, "
    ...
    if connection.scalar(sa.select(sa.func.count()).select_from(bindings)) or connection.scalar(
        ...
        raise RuntimeError(
            "cannot downgrade after durable Alpaca paper account bindings have been persisted"
        )
    op.drop_index(
        "ix_phase4_alpaca_account_binding_heads_valid_until",
    ...
    op.drop_index(
        _INGRESS_PARENT_INDEX,
        table_name="phase4_broker_ingress_receipts",
    )
