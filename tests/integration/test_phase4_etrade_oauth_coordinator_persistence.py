...
def test_sqlite_concurrent_conflicting_advancements_have_exactly_one_winner(
    ...
    received, guard = _request_token_advance(initial, current.replay_guard)
    advanced = repository.advance(current, received, guard)
    alternate_guard = EtradeOAuthReplayGuard(
        consumed_fingerprints=(*guard.consumed_fingerprints, "e" * 64),
        signing_time_high_waters=guard.signing_time_high_waters,
    )
    candidates = (
        (begin_etrade_oauth_out_of_band_authorization(received), guard),
        (received, alternate_guard),
    )
    barrier = Barrier(2)
    ...
    def attempt(candidate: tuple[EtradeOAuthSessionState, EtradeOAuthReplayGuard]) -> str:
        barrier.wait()
        try:
            repository.advance(advanced, *candidate)
    ...
    reloaded = repository.load(EtradeEnvironment.SANDBOX, EtradeSecretScope.SANDBOX_CONSUMER)
    assert reloaded.sequence == 3
    assert reloaded.state in tuple(candidate[0] for candidate in candidates)
