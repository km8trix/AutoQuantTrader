...
def test_public_material_has_the_exact_fixed_stop_scope_and_key_identity() -> None:
    ...
    assert authority.payload() == {
        ...
        "status": "public_graceful_stop_operator_authority_material",
    }
    assert set(authority.payload()) == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_FIELDS
    assert (
        authority.payload()["algorithm"]
        == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_ALGORITHM
    )
    assert (
    ...
    assert authority.payload()["key_id"] == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_KEY_ID
    assert (
        authority.payload()["replay_domain"]
        == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_REPLAY_DOMAIN
    )
    assert (
        authority.payload()["service"] == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_SERVICE
    )
    assert authority.payload()["status"] == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_STATUS


def test_stop_identity_is_never_the_start_authority_identity_or_type() -> None:
    ...
    assert type(stop_authority) is TrustedTimePostEnrollmentGracefulStopOperatorAuthority
    assert start_authority.__class__.__qualname__ != stop_authority.__class__.__qualname__
    assert stop_authority.encoded != canonical_post_enrollment_operator_authority_bytes(
        start_authority
    )

    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        build_post_enrollment_graceful_stop_operator_authority(start_authority)
    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        canonical_post_enrollment_graceful_stop_operator_authority_bytes(start_authority)
    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        decode_post_enrollment_graceful_stop_operator_authority(
            canonical_post_enrollment_operator_authority_bytes(start_authority)
        )
...
def test_construction_and_decode_reject_non_prime_subgroup_encodings_with_stop_error(
    reason: str,
    ...
    with pytest.raises(
        ...
        build_post_enrollment_graceful_stop_operator_authority(public_key_bytes)
    assert not isinstance(builder_error.value, TrustedTimePostEnrollmentOperatorAuthorityError)

    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        TrustedTimePostEnrollmentGracefulStopOperatorAuthority(public_key_bytes=public_key_bytes)
    ...
    with pytest.raises(
        ...
        decode_post_enrollment_graceful_stop_operator_authority(_encoded(payload))
...
def test_builder_and_value_object_reject_malformed_key_types_and_lengths(
    ...
    with pytest.raises(
        ...
        TrustedTimePostEnrollmentGracefulStopOperatorAuthority(
            public_key_bytes=cast(Any, public_key_bytes)
        )
...
def test_manifest_is_structurally_verification_only_and_grants_no_authority() -> None:
    ...
    assert b"post-enrollment-start/operator-attestation" not in encoded
...
def test_canonical_round_trip_and_artifact_sha_are_exact() -> None:
    ...
    assert decode_post_enrollment_graceful_stop_operator_authority(expected_encoded) == authority
...
def test_payload_copy_and_value_object_copies_are_isolated() -> None:
    authority = _authority()
    first_payload = authority.payload()
    second_payload = authority.payload()
    shallow_copy = copy.copy(authority)
    deep_copy = copy.deepcopy(authority)

    assert first_payload == second_payload
    assert first_payload is not second_payload
    first_payload["status"] = "mutated"
    assert authority.payload()["status"] == POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_STATUS
    assert shallow_copy == authority and shallow_copy is not authority
    assert deep_copy == authority and deep_copy is not authority

    object.__setattr__(shallow_copy, "public_key_bytes", b"short")
    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        canonical_post_enrollment_graceful_stop_operator_authority_bytes(shallow_copy)
    assert authority.public_key_bytes == PUBLIC_KEY_BYTES
    assert deep_copy.public_key_bytes == PUBLIC_KEY_BYTES
...
@pytest.mark.parametrize(
    ...
    [
        ...
        b'{"value":NaN}\n',
        b"\xff\n",
        b" " * (POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_MAXIMUM_ARTIFACT_BYTES + 1),
    ],
)
def test_decoder_rejects_malformed_container_types_json_and_size(encoded: object) -> None:
    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        decode_post_enrollment_graceful_stop_operator_authority(encoded)
...
def test_decoder_rejects_duplicate_extra_and_missing_fields() -> None:
    ...
    for candidate in (duplicate, _encoded(extra), _encoded(missing)):
        with pytest.raises(
            ...
            decode_post_enrollment_graceful_stop_operator_authority(candidate)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "contract_version",
            "phase6d-post-enrollment-graceful-stop-operator-attestation-authority-v2",
        ),
...
@pytest.mark.parametrize(
    "field_name", sorted(POST_ENROLLMENT_GRACEFUL_STOP_OPERATOR_AUTHORITY_FIELDS)
)
...
def test_decoder_rejects_noncanonical_or_wrong_length_base64(
...
def test_canonical_helpers_revalidate_exact_object_type_and_key_shape() -> None:
    with pytest.raises(
        ...
        canonical_post_enrollment_graceful_stop_operator_authority_bytes(object())
    with pytest.raises(
        TrustedTimePostEnrollmentGracefulStopOperatorAuthorityError, match="invalid"
    ):
        post_enrollment_graceful_stop_operator_authority_artifact_sha256(object())
