...
# evaluations without granting scheduler, control, broker, exposure, or re-arm
# authority.  Every process epoch starts at evaluation sequence zero; only a
# process-local repository session may extend it.
phase6_trusted_time_epoch_registrations = sa.Table(
    ...
    sa.CheckConstraint(
        ...
        "AND length(policy_sha256) = 64 "
        "AND policy_sha256 = "
        "'e2ed2efe97b6a13764fba36976916001eec074773f1f2fcf37f759c80e474944' "
        "AND length(semantic_sha256) = 64 "
        "AND (previous_monitor_epoch_id IS NULL "
