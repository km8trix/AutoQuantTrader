...
fail-closed captured-tape validity gate, read-only governance inspection,
bounded authenticated fixture-job/transcript provenance views, and one
internal exact-decimal economic receipt executed by a fixed repository-owned
child under bounded process/resource isolation. That Phase 3H path accepts
only the exact completed Phase 3F reference fixture and fixed zero-cost model;
arbitrary or fitted code, captured tapes, general economics, persistence,
API/reporting, criteria adjudication, promotion, shadow, and broader research-
UI work are not complete. The repository does not
implement the target paper/live topology below. ADR 0096 selects E\*TRADE
production as the future live venue without granting authority. The Phase
4A-4AI chronology that follows remains immutable historical Alpaca paper
evidence, not a live-adapter recommendation; Phases 4AJ-4AN are separate bounded
E\*TRADE foundations and prerequisites, not a deployed adapter. Phase 4A supplies an offline,
...
complete secret-free local replay/session history. Phase 4AN composes an exact
authenticated store head and fresh replay-only reservation with injected
ephemeral secret resolution, signing, a sealed injected token transport,
raw-first secret custody, strict decode, and a proposed request/access-token
successor. It deliberately does not commit that successor: no secret store can
atomically bind token values to the proposed nonsecret reference. This remains
non-authorizing local composition only. No deployed or concrete secret
resolver, TLS/network transport, provider caller, browser/OOB or deployed
session runtime, token store, successful session-head commit, production
caller, general security master, stream, reconciliation, order effect, or
dispatch authority exists. The trader
remains `not_ready`, Phase 4's exit gate is open, and no network-capable order
transport or production market-data adapter is enabled. No authenticated real
E\*TRADE token response, deployed OAuth session manager, account binding,
...
Phase 6 remains open. ADR 0125 completes ADR 0121 milestone one as an
unreachable injected lifecycle-v2 implementation, and Wave 7 now implements ADR
0126's milestone-two owner foundation on the local integration branch. Native
pre-Python fork ownership, role-narrow signers, pathname Unix-seqpacket
endpoints and resource admission, fixed credential provisioners, tmpfs-only
secret custody, source-only systemd topology, and four canonical x86_64 seccomp
...
projection, unit installation/activation, deployment, or stop authority.
`trusted-time-stop` remains the exact exit-2 no-effect target and the trader
remains `not_ready`. Milestone three must provide isolated injected real-root
and Docker composition, operational import/dependency composition, signed
socket/process-epoch evidence including boot UUID, executable/import hashes,
nonce, and immutable image, bounded `/proc/<pid>/fd` and `/proc/net/unix`
pre/post channel-closure proof, and arbitrary-Python-compromise safety.
Milestone four retains immutable production release, deployment, drills,
activation, and every stop-authority change. Phases 7 and 8 remain
operationally ahead.
...
[ADR 0123](adr/0123-injected-etrade-oauth-token-runtime-prerequisite.md)
implements Phase 4AN as an injected, non-authorizing prerequisite composition
for only request-token and access-token exchange. The runtime starts from a
fully authenticated Phase 4AM snapshot and a store-issued, process/thread-bound
one-use reservation. Before secret resolution or dispatch, that reservation
atomically appends one fresh signing replay-only fact while leaving the
sanitized session state unchanged. Stale, cross-store, reused, or concurrent
claims fail before any secret or transport boundary.

The resolver and transport are fixed injected ports with no production
implementation. Secret material enters only a sealed, one-use, explicitly
closable mutable envelope. The runtime signs through the Phase 4AL boundary and
presents a sealed request capability to a synthetic transport. Exact response
bytes are retained before decode inside private mutable custody, under a closed
origin/status/media/charset/redirect/proxy/TLS-declaration profile; those
declarations prove only the injected contract and do not authenticate a real
TLS channel or provider. Successful strict form decoding yields a sealed
ephemeral result containing the replay-only snapshot, proposed pure session
successor, nonsecret output reference, and sanitized receipt. Raw bytes, token
values, signatures, headers, verifier material, and secret-derived digests are
excluded from ordinary evidence and overwritten on close as defense in depth.

Phase 4AN intentionally does not advance the proposed successor. The slice has
no secret store capable of atomically committing token values before a durable
head names their reference, so post-transport/pre-store ambiguity remains an
explicit blocker. It adds no schema or migration, deployed/concrete resolver,
network/TLS client, authenticated provider response, browser/OOB handoff,
deployed session runtime, renewal/revocation scheduler, production caller,
account binding, broker action, startup, or authority. A real token runtime and
successful session-head commit require separate reviewed secret-store,
transport/origin, custody, operator-handoff, and recovery composition.
...
### Current Phase 3H bounded fixture economic-segment isolation

Phase 3H accepts only an exact, fully revalidated `COMPLETED` Phase 3F job
projection and the exact certified target replay named by that job. It
cross-binds the authenticated job, family, attempt, configuration, segment,
completion receipt, target artifact, certification, and transcript roots before
any child process can start. Queued, running, failed, corrupt, or substituted
evidence fails before launch.

The sole model is `immediate-causal-close-zero-cost-v1`: exactly USD 100,000
initial cash, immediate long-only whole-share fills at each READY decision's
same causal close, position carry through WAITING rows, no costs or external
cash flows, and a final mark at the last complete row. Requests are bounded to
2,048 complete rows, 64 canonically ordered instruments, and 256 KiB. Decimal
arithmetic follows the repository's exact 64-digit/e63 policy; floats cannot
enter the request or result. The parent independently recomputes ending cash,
market value, equity, net P&L, gross traded notional, counts, and final
positions before it can issue a content-addressed receipt.

Execution uses exactly one reviewed repository-owned stdlib child, snapshotted
to an unlinked read-only descriptor and invoked through fixed isolated argv,
working directory, and environment. The child must apply and read back the
closed CPU, descriptor, core, file-output, child-process, and platform-specific
address-space limits before reading input. The parent separately enforces the
wall-time and pipe bounds, kills only the new process group on failure, requires
a strict canonical JSON protocol plus distinct PID/session evidence, and
sanitizes every child/supervisor fault. This is fixed-program isolation, not a
hostile-code sandbox.

The receipt remains internal and recomputable. Phase 3H adds no table,
migration, repository write, API, CLI, browser/report field, Phase 3G view,
criteria or promotion surface, worker/entry-point reachability, provider I/O,
or trading authority. Arbitrary or fitted code, caller-selected fixtures or
economics, captured tapes, costs/slippage/benchmarks, walk-forward and
statistical evaluation, general economic execution, persistence, criteria
adjudication, promotion, and deployment remain open. See
[ADR 0122](adr/0122-bounded-economic-segment-execution-isolation.md).
...
deadline expiry, process loss, or failed reauthentication after reservation is
recovery-required and never an automatic retry. Generic stopped status, process
...
consumed-snapshot handoff remains bound to the v1 bridge. ADR 0121 chooses an
additive private ADR-0112 consumer bound to an independent v2 bridge capability,
operation, admission, channel, PID, and Thread. It reuses the reviewed stable
historical-source reconstruction, not the v1 bridge identity or a second
historical loader; every existing ADR-0112 public and v1-private meaning stays
unchanged.

ADR 0121 resolves the implementation choices without adding runtime state or
authority. Transport is Linux `AF_UNIX` `SOCK_SEQPACKET` at the fixed
root-created tmpfs path
`/run/autoquant/trusted-time/graceful-stop-v2/transport/supervisor.sock`.
The transport tmpfs is exactly `0:10001:0770`, privately projected read-write
into only the UID/GID-10001 supervisor, whose socket is `10001:10001:0600`;
`0750`, read-only, extra-projection, owner/mode, and path/inode drift reject.
Docker's private PID namespace remains intact. The host fully maps the positive
supervisor `SO_PEERCRED` PID/start/cgroup/container identity; the supervisor
requires the outside-namespace host peer as UID/GID `0:0`, PID zero and relies
on the host-key-signed exact process epoch rather than attempting `/proc/0`.
User-namespace remap is forbidden by future admission.

Role-separated Ed25519 keys, authenticated by one non-expiring offline-root-
signed transport manifest and one exact root-signed generation/recovery
selection record, sign the full canonical nonsecret request/result/error
envelopes. The pinned decrypt profile writes each TPM-bound blob directly to
one preopened inode in the named role tmpfs; `LoadCredentialEncrypted` and any
second plaintext path are forbidden. Exact host hello, supervisor hello, and
host confirmation contracts
bind full process-epoch objects, fresh process/channel nonces, both endpoint and
peer views, and a deterministic domain-separated channel preimage before any
...
deadline equality fail closed. Connect/handshake is bounded to five seconds,
...
V2 preserves `.post-enrollment-graceful-stop-attempt-slot` as the one permanent
v1/v2 lifecycle and replay root. Its independent canonical root, progress,
transcript, outcome, commit, request, result, and error contracts form one
gap-free typed family; an exact v1 root consumes the slot, and every unknown or
mixed v1/v2 root, prefix, directory, or object graph is retention-unconfirmed.
Each immutable content-addressed transcript snapshot has an exact root/progress
entry list and a domain-separated digest; exclusive staging, no-replace rename/
fsync, and readback are its publication commit. Ordinal two first publishes the
complete canonical signed result-or-error transport envelope bytes—including
payload and signature—as the one immutable content-addressed wire artifact
under `<ignored-root>/trusted-time`; ordinary SHA-256 of those exact bytes
selects the literal result/error filename. Exclusive no-follow staging, file
and directory fsync, no-replace rename, stable readback/re-encode/signature/hash
verification, and an exact nested publication receipt precede ordinal-two
...
correlators, a complete nested request, a closed twenty-field ADR-0108 primitive
projection, and a complete signed supervisor-cleanup commitment rather than a
prose or giant-object schema. The normal path retains
request intent/result, transport cleanup commitment/quiescence at ordinals three/
four, pre-effect reauthentication, supervisor stop, source stop, per-ID
container and network removals, two-volume preservation, distinct post-teardown
reauthentication, terminal cleanup intent/result at twenty-one/twenty-two, and
only then confirmed outcome ordinal twenty-three. Every
CALL has a dedicated durable pre-CALL intent and separately authenticated
post-CALL result. Missing or ambiguous evidence is never retry permission.

The existing native global launcher lease is the only host cross-process lock
and remains held from authenticated channel preflight through four-way fresh
admission, root reservation, every STORE/CALL pair, transport and terminal
cleanup, final transcript, and the fixed outcome commit. Signed transport
cleanup commitment, EOF/path/inode plus stable
...
socket, effect, observation, or outcome candidate, cannot invalidate or
reclassify the fixed commit, and is guaranteed to release the flock on kernel
process exit. The supervisor never acquires that flock, and v2 adds no second
repository flock. A native pre-Python `pthread_atfork` guard
invalidates and closes a fixed owner table in the child before any registry
lock; every entrypoint checks PID, exact Thread, and fork epoch before locking.
Process creation is denied while v2 ownership is live. Effects therefore use a
method-narrowed Docker Engine `/v1.45` client over the admitted exact Unix
socket, not Docker CLI or Compose. It can stop/delete only admitted container
IDs, delete only the admitted network ID, cannot delete volumes, and must prove
both exact named-volume identity projections unchanged with a retained zero-
volume-delete trace. ADR 0121 freezes every literal inspect/stop/delete target,
including `POST .../stop?t=30`, explicit preserving container-delete flags
`v=false&force=false&link=false`, empty network-delete query/body, and exact-ID
GET paths. Each durable intent, result, and method-trace entry binds the full
method/path/ordered-query/fixed-header/absent-body digest; any alternate query,
header, body, signal, or path fails before socket I/O.
Responses reject non-HTTP/1.1, chunking, invalid/duplicate framing, length/body
drift, or endpoint byte-ceiling excess; 204 is zero-body and normal delete
post-inspects require the exact one-field 404 JSON. `/info`, container, network,
and volume bodies are duplicate-aware bounded JSON decoded into exact allowlisted
projections. Unknown raw fields are explicitly ignored only after that parse and
remain bound by the raw-body digest; framing, body, and projection digests all
enter each result and trace entry. Same-lock admission performs exactly six
Docker reads in order: `/info`, supervisor full-ID inspect, source full-ID
inspect, project-network full-ID inspect, command-socket-volume inspect, then
state-volume inspect. The admission/root binds the ordered request, complete
connection-identity, HTTP-exchange, and trace objects and their digests.
Thereafter, one fresh connection serves each fixed call in the literal ordinal
sequence `6..17`. Each canonical connection identity binds the socket
mount/path/device/inode, root peer credential and daemon process identity,
local socket device/inode/`SO_COOKIE`, call ordinal, admitted daemon projection,
and capture/revalidation times; a raw FD number is never authority. Closed
container-stop, container-remove, network-remove, and volume-preservation
result objects embed complete primary/post-inspect connection and exchange
objects, complete trace-entry objects, and parallel request/connection/exchange/
trace digests plus HTTP/framing/body/projection digests, timestamps, and exact
outcome. Reorder, implicit reads, identity drift, retry, or a digest-
only result is recovery-required.

Fresh ADR-0109 observations are consumed by separate v2 pre-effect and terminal
binding seams. The second observation must start after durable teardown and
volume proof and cannot reuse the first issuer, object, interval, or binding.
...
ADR 0124 remains the historical record of the partial Wave 5 core. ADR 0125
completes ADR 0121 milestone one without activating it. The independent v2
domain now provides canonical authority manifest/selection, boot/process/peer/
socket, handshake/channel, request/result/error, terminal, Docker HTTP, root,
progress/transcript, outcome, commit, and recovery-classification families with
strict v1/v2 rejection. Verifier-owned Ed25519 seams authenticate authority,
frames, terminal payloads, and retained signed-wire bytes without owning a
private key or opening a socket.

The injected repository and descriptor-safe physical store enforce the one
shared v1/v2 slot, exact owner and path/inode identity, bounded inventory,
exclusive/fsync/no-replace/readback publication, immutable signed-wire and
transcript relationships, and ambiguity burns. Named typed transitions bind
the complete clean-stop/Docker evidence, exact pre-CALL intents and post-CALL
results, complete canonical ADR-0109 binding/observation evidence at ordinals 6
and 20, transport and terminal cleanup, and the normal lineage through ordinal
22. Confirmed success at ordinal 23 consumes only the exact sealed retained
lineage, disposes success-relevant descriptors and registry state, passes the
last equality-expired authorization check, and publishes only the fixed marker.

The test-only byte-level daemon exercises exact `/v1.45` requests and bounded
raw HTTP responses across ordinals `0..17`, including framing, projection,
identity, disconnect, replay, and ambiguity faults, without opening the Docker
socket. Authenticated recovery is generation/root/prefix/nonce/PID/Thread-bound,
one use, and classification-only: it can retain deterministic
`recovery_required` state or finalize only an exact already-created marker
preimage, never dispatch transport or mutate Docker.

ADR 0126 now adds the local milestone-two native owner foundation while
preserving the zero-production-caller boundary. The fixed pre-Python fork
owner, role-constrained signers, pathname Unix-seqpacket endpoint/resource
owners, tmpfs-only secret custody, and fixed provisioners are separately
compiled into six reproducible, uninstalled candidates. Four canonical x86_64
seccomp manifests and source-only, uninstalled systemd units freeze the process
and resource topology. Candidate builders and native source evidence are
source-distribution-only and excluded from wheels; immutable inert role import
trees are bound into the candidate evidence; and every record states
`activation_authorized=false`.

Recovery binds a separate exact minimal bootstrap standard-library tree with
only its required encoding modules and the Python license, one immutable inert
entry root, and no dynamic-extension load path. Libpython built-ins are
inventoried rather than claimed absent; the no-input inert entry does not reach
them, and seccomp blocks effects. This is packaging and static isolation
...
exit-2 no-effect target. Milestone three owns isolated injected real-root/Docker
and operational import/dependency composition, signed socket/process-epoch
evidence including boot UUID, executable/import hashes, nonce, and immutable
image, bounded `/proc/<pid>/fd` and `/proc/net/unix` pre/post channel-closure
proof, and arbitrary-Python-compromise safety. Milestone four owns immutable
production release, deployment, drills, activation, and every stop-authority
change. See
[ADR 0124](adr/0124-trusted-time-lifecycle-v2-milestone-one.md),
[ADR 0125](adr/0125-complete-injected-trusted-time-lifecycle-v2-milestone-one.md),
and
[ADR 0126](adr/0126-native-trusted-time-lifecycle-v2-milestone-two-owner-foundation.md).
...
consume ADR 0125's additive v2-bound ADR-0112 historical-receipt handoff and
independent v2 request/result/error/lifecycle family, never the current
ADR-0112-to-ADR-0111 v1 handoff, and satisfy the five gates above plus ADR 0121's
remaining rollout milestones. Transport cannot be activated before same-lock
admission, lifecycle-v2 ambiguity retention, native fork-safe ownership, exact
real key/endpoint admission, and classification-only recovery are jointly
admitted. The structural v2 result must be freshly cross-bound through the v2
host seam to a pre-effect ADR-0109 observation before any stop/teardown effect,
and no success can precede exact supervisor-first/source-second effects,
ID-bound container/network teardown, both-volume preservation, a distinct post-
teardown terminal reauthentication through its separate v2 seam, and durable
outcome publication.
