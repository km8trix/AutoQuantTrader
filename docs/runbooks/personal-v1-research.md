...
Resource profile `posix-cpu-file-wall-sampled-resident/2` reads Linux's current-address-space `VmHWM` from a bounded `/proc/self/status` read for the final peak check; Darwin retains its process `getrusage` check. Linux [`getrusage` preserves usage across exec](https://man7.org/linux/man-pages/man2/getrusage.2.html) and can therefore reject a small child because of memory used before its current executable started. Missing or malformed peak observations prevent publication. Live RSS sampling and Linux's address-space limit remain enforced. The [Linux proc documentation](https://docs.kernel.org/filesystems/proc.html) defines `VmHWM` and notes the limits of kernel RSS accounting; this remains a conservative measured resource profile. Existing reports retain their original resource and source pins.
