...
> [Architecture](docs/ARCHITECTURE.md) and [implementation waves](docs/IMPLEMENTATION_PLAN.md) are the current design and roadmap. Wave 1 adds a Tiingo research import and a halted standard runtime; E*TRADE account reads still require owner OAuth consent. See the [foundation runbook](docs/runbooks/personal-v1-foundations.md).
...
Follow the [foundation runbook](docs/runbooks/personal-v1-foundations.md) to create a separate environment, run `make personal-simulation`, or import a dataset. Consumer credentials are configured for both E*TRADE environments, but the interactive OAuth helper is ready for owner consent, and actual account-read qualification remains pending.
