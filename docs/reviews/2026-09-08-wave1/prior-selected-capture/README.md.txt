...
> [Architecture](docs/ARCHITECTURE.md) and [implementation waves](docs/IMPLEMENTATION_PLAN.md) are the current design and roadmap. Wave 1 adds a Tiingo research import and a halted standard runtime; E*TRADE OAuth and account discovery succeeded; full account capture awaits the owner’s account selection. See the [foundation runbook](docs/runbooks/personal-v1-foundations.md).
...
Follow the [foundation runbook](docs/runbooks/personal-v1-foundations.md) to create a separate environment, run `make personal-simulation`, or import a dataset. Owner OAuth and account discovery succeeded for sandbox and production. Full account-read qualification awaits exact account selection and the owner’s exclusivity declaration.
