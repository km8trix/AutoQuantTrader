...
Wave 2 local exit gates passed on codex/personal-v1-w2-integration from that merged base. Read canonical plan section 17 and docs/reviews/2026-09-09-wave2/README.md. The one causal engine, retained accounting port and derived report are implemented. Four 520-session runs pass independent rational/annual-statistic verification ; 1,633 Python tests pass with 7 PostgreSQL skips ; source and installed-wheel process recovery pass ; two ordinary wheels are byte-identical. The general worker replaces the default golden loop, which is preserved under an explicit oracle entry/profile. Finish PR/required CI/review/merge and exact merged-tree verification before starting W3. At most3 additional workers globally; no nested workers. No provider calls or deployment are needed for current work.

The account amendment permits explicitly selected dedicated CASH or MARGIN accounts, while strategy funding remains cash-funded, long-only and without borrowing. Production read traversal passed; sandbox balance identity remains blocked. Currency, liability/restriction/cash semantics, quote rights/freshness, actual quotas, recovery identities and reconciliation remain connected-execution gates. Trading authority is false. W2 is offline research work and requires no provider calls or credentials. Never display keys, tokens, private session paths or provider payloads. Owner authorization for earlier reads does not authorize orders or deployment.
